...
internal sealed class IosPhotoImporter : IPhotoImporter
{
   ...
   public async Task<string[]> ImportAsync()
   {
      ...
      if (status == PermissionStatus.Granted)
      {
         _assets = PHAsset.FetchAssets(PHAssetMediaType.Image, null)
            ...
            .ToDictionary(asset =>
                  asset.ValueForKey((NSString)"filename").ToString(), asset => asset
            );
      }
   ...
   private void AddImage(ObservableCollection<Photo> photos, string path, PHAsset asset, Quality quality)
   {
      ...
      PHImageManager.DefaultManager.RequestImageForAsset(
         asset, PHImageManager.MaximumSize, PHImageContentMode.AspectFill, options, (image, _) =>
         {
