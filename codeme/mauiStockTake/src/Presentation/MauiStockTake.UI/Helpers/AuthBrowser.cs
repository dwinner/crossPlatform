...
public class AuthBrowser : IdentityModel.OidcClient.Browser.IBrowser
{
    ...
    {
#if WINDOWS
        var authResult = await WinUIEx.WebAuthenticator.AuthenticateAsync(new Uri(options.StartUrl), new Uri(Constants.RedirectUri));
        ...
        string response = $"{Constants.RedirectUri}#code={code}&scope={scope}&state={state}&session_state={sessionState}";
