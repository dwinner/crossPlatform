...
{
   private const string ModelName = "version-RFB-320.onnx";
   private const string SourceImageName = "test_image.jpg";
   ...
   {
      _sourceImagePath = await FileOperations.CopyToAppData(SourceImageName);
      var onnxModelPath = await FileOperations.CopyToAppData(ModelName);
   ...
   {
      IEnumerable<ImageNetData> images = new List<ImageNetData>
      {
   ...
   private SKBitmap CreateCroppedBitmap(SKBitmap originalBitmap, SKRectI cropRect)
   {
      ...
      canvas.DrawBitmap(
         originalBitmap, cropRect, new SKRectI(0, 0, cropRect.Width, cropRect.Height)
      );
