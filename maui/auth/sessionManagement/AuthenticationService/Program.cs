   ...
   RoleManager<IdentityRole> roleManager) =>
{
   ...
   var data2 = JsonSerializer.Deserialize<JsonNode>(responseText);
   var token = data2["accessToken"].GetValue<string>();
   var refreshToken = data2["refreshToken"].GetValue<string>();
   var expiresIn = data2["expiresIn"].GetValue<int>();
...
app.MapPost("/registerUser", async (
   ...
   await userManager.CreateUserWithRoleAsync(
      roleManager, email, password, new DateOnly(2000, 1, 1), "User")
);

app.MapDelete("/users/{email}", async (
   string email,
   UserManager<User> userManager,
   IAuthorizationService _,
   HttpContext _) =>
{
