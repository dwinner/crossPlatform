...
public partial class CustomerEditViewModel : ObservableObject, IQueryAttributable
{
   ...
   private Func<Customer, Task> ParentRefreshAction { get; set; }

   public void ApplyQueryAttributes(IDictionary<string, object> query)
   {
      if (query.TryGetValue("Item", out var currentItem))
      {
         Item = (Customer)currentItem;
      }

      if (query.TryGetValue("ParentRefreshAction", out var parentRefreshAction))
      {
