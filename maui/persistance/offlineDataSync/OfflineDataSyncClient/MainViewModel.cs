...
public partial class MainViewModel : ObservableObject
{
   ...
   {
      var queuedBlogs = context.DatasyncOperationsQueue.Where(syncOp => syncOp.EntityType == typeof(Blog).FullName);
   ...
   private async Task AddBlog()
   {
      ...
      await SaveChangedItem(newBlog);
   }

   private bool CanAddBlog() => !string.IsNullOrEmpty(NewItemText);

   private async Task SaveChangedItem(Blog newBlog)
   {
      await using var context = new LocalAppDbContext();
