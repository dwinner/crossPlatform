...
public class Stations
{
    ...
    {
        _time = time ?? new DefaultTimeAbstraction();
    }
    ...
    [
        ...
        new Station(AirportTerminusId, AirportTerminusName, Branch.MainLine, (109.4f, 81.2f), true),
        ...
        new Station(208, NorthBranchTerminusName, Branch.NorthBranch, (10f, 10.25f), true),
        ...
        new Station(306, SouthBranchTerminusName, Branch.SouthBranch, (8.8f, 128.2f), true)
    ];
    ...
    public List<TrainInfo> GetCurrentTrainsInNetwork()
    {
        ...
        if (now.TimeOfDay >= startOfDay)
        {
            ...
            while (loopMins <= now.TimeOfDay.TotalMinutes)
            {
                ...
                {
                    void AddInfoForTrainLeavingTerminus(int terminusId, CompassDirection direction,
                        bool headingNorth = false)
                    {
                        var (left, approaching) =
                            StationsBetweenAfterLeavingTerminus(terminusId, (int)timeSinceDeparture, headingNorth);
                        ...
                        results.Add(new TrainInfo
                        {
                            ...
                            MapPosition = GetMapPoints(left, approaching,
                                (int)timeSinceDeparture - MinutesFromTerminus(left, terminusId))
                        });
                    }
                    ...
                    if (loopMins % 30 == 0)
                    {
                        ...
                        heading = timeSinceDeparture > minsToReachMainBranch
    ...
    public (float X, float Y) GetMapPoints(int stnLeft, int stnApproaching, int minsSinceLeft)
    {
        ...
        var minsBetweenStns = Math.Abs(MinutesFromTerminus(stnLeft, AirportTerminusId) -
                                       MinutesFromTerminus(stnApproaching, AirportTerminusId));
    ...
    private List<ArrivalDetail> GetNextEastboundArrivalDetails(Station station, DateTimeOffset now,
        TimeSpan workingTime, TimeSpan startOfDay) =>
        GetNextArrivalDetailsLogic(station, now, workingTime, startOfDay, 15, null, AirportTerminusName);

    private List<ArrivalDetail> GetNextWestboundArrivalDetails(Station station, DateTimeOffset now,
        TimeSpan workingTime, TimeSpan startOfDay) =>
        GetNextArrivalDetailsLogic(station, now, workingTime, startOfDay, 15, AirportTerminusId, null);

    private List<ArrivalDetail> GetNextNorthboundArrivalDetails(Station station, DateTimeOffset now,
        TimeSpan workingTime, TimeSpan startOfDay)
    {
    ...
    private List<ArrivalDetail> GetNextSouthboundArrivalDetails(Station station, DateTimeOffset now,
        TimeSpan workingTime, TimeSpan startOfDay)
    {
    ...
    private List<ArrivalDetail> GetNextArrivalDetailsLogic(Station station, DateTimeOffset now, TimeSpan workingTime,
        TimeSpan startOfDay, int minsBetweenTrains, int? sourceStationId, string destStationName)
    {
    ...
    internal (int left, int approaching) StationsBetweenAfterLeavingTerminus(int terminusId, int minutes,
        bool headingNorth = false)
    {
        switch (terminusId)
        {
